...
1. [**[2001] Bernard J. Jansen, Udo Pooch, _A review of Web searching studies and a framework for future research_**](https://onlinelibrary.wiley.com/doi/epdf/10.1002/1097-4571%282000%299999%3A9999%3C%3A%3AAID-ASI1607%3E3.0.CO%3B2-F):
    ...
    - Extensive review and analysis of current Web-searching studies
...
2. [**[2003] In-Ho Kang, GilChang Kim, _Query Type Classification for Web Document Retrieval_**](https://www.cs.cmu.edu/~ihkang97/papers/query_type.pdf):
    ...
    - Features for building query type classifier:
        ...
        - Mutual Information (to capture the semantic information between 2 frequently co-occuring words in a query)
...
3. [**[2005] Uchin Lee, Zhenyu Liu, Junghoo Cho, _Automatic Identification of User Goals in Web Search_**](http://oak.cs.ucla.edu/~cho/papers/cho-usergoal.pdf):
    ...
    - Categories of features for the automatic identification of the user goal:
        ...
        - Anchor-Linkage Distribution:
            - `highly skewed towards rank 1 => navigational`
...
4. [**[2006] Gary Marchionini, _Exploratory Search: From Finding to Understanding_**](https://dl.acm.org/doi/fullHtml/10.1145/1121949.1121979):
    ...
    - Exploratory intent searches, may require strong human participation to support user with continuous & exploratory process
...
5. [**[2008] Doug Downey, Susan Dumais, Dan Liebling, Eric Horvitz, _Understanding the Relationship between Searchers’ Queries and Information Goals_**](https://dl.acm.org/doi/abs/10.1145/1458082.1458143):
    ...
    - TaskDifficulty = - log<sub>2</sub>( P(goal URL) ) , where P(goal URL) = freq. of URL satisfying the user's goal, expressed as probability
...
6. [**[2009] Michael Bendersky, W. BruceCroft, _Analysis of Long Queries in a Large Scale Search Log_**](https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.323.7444&rep=rep1&type=pdf)
    - [TREC corpora](https://trec.nist.gov/data/kba.html) used for analysis
    - Why to study _long_ queries [5 <= len(query) <= 12]:
        - Better way to represent complex & specific information  requirements
...
7. [**[2010] Anne Aula, Rehan M. Khan and Zhiwei Guan, _How does Search Behavior Change as Search Becomes More Difficult?_**](https://www.researchgate.net/publication/221516065_How_does_Search_Behavior_Change_as_Search_Becomes_More_Difficult):
...
8. [**[2010] Mauro Rojas Herrera, Edleno Silva de Moura a, Marco Cristo, Thomaz Philippe Silva, Altigran Soares da Silva, _Exploring features for the automatic identification of user goals in web search_**]():
    ...
    - Performance on experiments:
        - Much better best-case classification accuracies (82-90 %) across taxonomies on using best set of features from above on the  WT10g &  WBR03 collections
        - Navigational vs Informational classification:
            - SVM using all features __except__ terms, number of terms, anchor text frequency , match b/w query & domain
            - __94.87 Precision__ & __94.87 Recall__
        - Navigational vs Informational vs Transactional classification:
...
9. [**[2013] Carolyn Theresa Hafernik, Bernard J. Jansen, _Understanding the Specificity of Web Search Queries_**]()
    ...
    - List of 9 attributes to identify query specificity as _narrow_:
        - URL
        - location with additional item
        - compares multiple things
...
10. [**[2014] Kumaripaba Athukorala, Antti Oulasvirta, Dorota Głowacka, Jilles Vreeken, Giulio Jacucci, _Narrow or Broad? Estimating Subjective Specificity in Exploratory Search_**](https://eda.mmci.uni-saarland.de/pubs/2014/foraging-atukorala,oulasvirta,glowacka,vreeken,jaccuci.pdf):
    ...
    - Also trained __C4.5 Decision tree__ (10-fold cross validation)
...
11. [**[2015] Kumaripaba Athukorala, Dorota Głowacka, Giulio Jacucci, Antti Oulasvirta, Jilles Vreeken, _Is Exploratory Search Different? A Comparison of Information Search Behavior for Exploratory and Lookup Tasks_**](https://asistdl.onlinelibrary.wiley.com/doi/epdf/10.1002/asi.23617):
...
12. [**[2017] Emilie Palagi, Fabien Gandon, Raphaël Troncy, Alain Giboin, _A Survey of Definitions and Models of Exploratory Search_**](http://www.eurecom.fr/en/publication/5165/download/data-publi-5165.pdf)
    - Exploratory searching behavior is similar to _information foraging_ (move from one information patch to another based on _information scent_) or _berrypicking_
...
13. [**[2018] Daniel Hienert, Matthew Mitsui, Philipp Mayr, Chirag Shah, Nicholas J. Belkin, _The Role of the Task Topic in  Web Search of Different Task Types_**](https://arxiv.org/ftp/arxiv/papers/1808/1808.06813.pdf)
    - Large number of session variables defined based on _task type_ for experiment
    - Session variables depend heavily on _task topic_, rather than simply  on _task type_.
...
14. [**[2019] Siddhartha Devapujula, Sagar Arora, Sumit Borar, _Learning to Rank Broad and Narrow Queries in E-Commerce_**](https://arxiv.org/pdf/1907.01549.pdf):
    ...
    - Randomly sampled 100k queries & manually labelled them as Broad/Narrow, & later split them into train & test data in the ratio 70:30
    ...
    - Features used:
        - Coherency score:
            ...
            - Typically, large recall set (broad queries) lead to smaller coherency scores
        - Word2Vec encodings
    ...
    - On another sample of 10k queries, __accuracy was 0.89__ (beating baseline heuristic-based classifier with accuracy 0.83)
...
- We can perhaps use the datasets mentioned in papers [2] & [8] above for testing our models as follows:
